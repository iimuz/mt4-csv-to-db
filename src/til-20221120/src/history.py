...
class History(Base):
    ...
    open_time = sa.Column(sa.DateTime(), nullable=False)  # 注文時間
    ...
    close_time = sa.Column(sa.DateTime, nullable=False)  # 決済時間
    close_price = sa.Column(sa.Float(8), nullable=False)  # 決済価格
    commission = sa.Column(sa.Float(8), nullable=False)  # 手数料
    expiration = sa.Column(sa.DateTime(), nullable=False)  # 有効期限
